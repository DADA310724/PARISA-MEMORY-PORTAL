...
async function fetchAndCacheChunk(id) {
    try {
        ...
        const chunk = { data: buf, contentType: ct, totalSize, ts: Date.now() };
        evictCache();
...
// ── Routes ───────────────────────────────────────────────────────────────────
driveRouter.get("/ready", async (_req, res) => {
    ...
    try {
        const token = await getAccessToken();
        const testResp = await fetch("https://www.googleapis.com/drive/v3/about?fields=user", { headers: { Authorization: `Bearer ${token}` } });
        if (testResp.ok) {
...
async function countFilesRecursive(folderId, token, depth = 0) {
    if (depth > 4)
        return 0;
    let total = 0;
    let pageToken;
...
driveRouter.post("/count-folders", async (req, res) => {
...
driveRouter.get("/file/:id", async (req, res) => {
    const { id } = req.params;
    try {
...
driveRouter.get("/text/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const token = await getAccessToken();
...
driveRouter.get("/prefetch/:id", async (req, res) => {
...
driveRouter.get("/stream/:id", async (req, res) => {
    const id = String(req.params["id"]);
...
driveRouter.get("/mediaurl/:id", async (req, res) => {
    const id = String(req.params["id"]);
...
driveRouter.post("/upload", upload.array("files"), async (req, res) => {
    ...
    try {
        ...
        const results = await Promise.all(files.map(async (f) => {
            ...
            body.append("file", new Blob([f.buffer], { type: f.mimetype }), f.originalname);
            const resp = await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,mimeType", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body });
            if (!resp.ok)
                return { error: await resp.text(), name: f.originalname };
            return resp.json();
        }));
        res.json({ results });
    }
...
driveRouter.post("/copy-from-drive", async (req, res) => {
    const { sourceFileId, destFolderId, name } = req.body;
    if (!sourceFileId || !destFolderId) {
        res.status(400).json({ error: "sourceFileId and destFolderId required" });
        return;
    }
    try {
        const token = await getAccessToken();
        ...
        if (name)
            body.name = name;
        const resp = await fetch(`https://www.googleapis.com/drive/v3/files/${sourceFileId}/copy?fields=id,name,mimeType`, {
            method: "POST",
...
driveRouter.get("/find-file", async (req, res) => {
    const folder_id = req.query.folder_id;
    const name = req.query.name;
    if (!folder_id || !name) {
        res.status(400).json({ error: "folder_id and name required" });
        return;
    }
    try {
...
driveRouter.get("/list-screenshots", async (req, res) => {
    const rootFolderId = req.query.folderId;
    if (!rootFolderId) {
        res.status(400).json({ error: "folderId required" });
        return;
    }
    try {
        ...
        const [subResp, rootImgResp] = await Promise.all([
            fetch((() => { const u = new URL("https://www.googleapis.com/drive/v3/files"); u.searchParams.set("q", `'${rootFolderId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`); u.searchParams.set("fields", "files(id,name)"); u.searchParams.set("pageSize", "20"); return u.toString(); })(), { headers: { Authorization: `Bearer ${token}` } }),
            fetch((() => { const u = new URL("https://www.googleapis.com/drive/v3/files"); u.searchParams.set("q", `'${rootFolderId}' in parents and mimeType contains 'image/' and trashed=false`); u.searchParams.set("fields", "files(id,name,modifiedTime)"); u.searchParams.set("orderBy", "modifiedTime desc"); u.searchParams.set("pageSize", "200"); return u.toString(); })(), { headers: { Authorization: `Bearer ${token}` } }),
        ]);
        if (!subResp.ok) {
            res.status(subResp.status).json({ error: await subResp.text() });
            return;
        }
        ...
        if (rootImgResp.ok) {
            const rootImgData = await rootImgResp.json();
            if (rootImgData.files.length > 0)
                result["__root__"] = rootImgData.files;
        }
        // Include subfolder images
        await Promise.all(subData.files.map(async (folder) => {
...
driveRouter.post("/create-folder", async (req, res) => {
    ...
    try {
        const token = await getAccessToken();
        const resp = await fetch("https://www.googleapis.com/drive/v3/files?fields=id,name,mimeType", {
            method: "POST",
            headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
            body: JSON.stringify({
                name,
