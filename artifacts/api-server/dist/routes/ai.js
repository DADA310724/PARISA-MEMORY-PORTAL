...
try {
    ...
    if (fp) {
        ...
        console.log(`✅ Chat DB — ${FLAT_DB.length} messages, ${JSON.parse(readFileSync(fp, "utf-8")).length} conversations`);
    }
}
catch (e) {
    console.warn("Chat DB load error:", e.message);
}
const ENGLISH_MONTHS = { january: "01", february: "02", march: "03", april: "04", may: "05", june: "06", july: "07", august: "08", september: "09", october: "10", november: "11", december: "12", jan: "01", feb: "02", mar: "03", apr: "04", jun: "06", jul: "07", aug: "08", sep: "09", oct: "10", nov: "11", dec: "12" };
const BENGALI_MONTHS = { জানুয়ারি: "01", ফেব্রুয়ারি: "02", মার্চ: "03", এপ্রিল: "04", মে: "05", জুন: "06", জুলাই: "07", আগস্ট: "08", সেপ্টেম্বর: "09", অক্টোবর: "10", নভেম্বর: "11", ডিসেম্বর: "12" };
...
function parseDateFilter(q) {
    ...
    for (const [mname, mnum] of Object.entries(ALL_MONTHS)) {
        if (q.includes(mname)) {
            ...
            if (yearOnly && day)
                return `${yearOnly[1]}-${mnum}-${day}`;
            if (yearOnly)
                return `${yearOnly[1]}-${mnum}`;
            if (day)
                return `-${mnum}-${day}`;
            return `-${mnum}-`;
        }
    }
    if (yearOnly && !q.match(/\d{5,}/))
        return yearOnly[1] ?? null;
    return null;
}
function getCtx(group, idx, win = 4) {
    return group.slice(Math.max(0, idx - win), Math.min(group.length, idx + win + 1));
}
function searchChatDB(query) {
    if (!FLAT_DB.length || !query.trim())
        return "";
    ...
    if (q.includes("whatsapp") || q.includes("ওয়াটসঅ্যাপ"))
        platformFilter = "whatsapp";
    else if (q.includes("messenger") || q.includes("মেসেঞ্জার"))
        platformFilter = "facebook messenger";
    else if (q.includes("telegram") || q.includes("টেলিগ্রাম"))
        platformFilter = "telegram";
    let pool = FLAT_DB;
    if (chatFilter)
        pool = pool.filter(m => m.chat_id.toLowerCase() === chatFilter);
    else if (platformFilter)
        pool = pool.filter(m => m.platform.toLowerCase().includes(platformFilter));
    const chatGroups = new Map();
    for (const m of pool) {
        if (!chatGroups.has(m.chat_id))
            chatGroups.set(m.chat_id, []);
    ...
    if (dateFilter) {
        ...
        if (dated.length > 0) {
            ...
            for (const m of dated.slice(0, 15)) {
                ...
                for (const c of getCtx(group, idx))
    ...
    for (const { msg } of matches.slice(0, 10)) {
        ...
        for (const c of getCtx(group, idx, 3))
            if (!results.includes(c))
                results.push(c);
...
function getEnvKeys(envVar) {
    ...
    for (let i = 2; i <= 20; i++) {
        ...
        if (val)
            val.split(",").map(k => k.trim()).filter(Boolean).forEach(k => keys.push(k));
    }
...
async function tryGroq(messages, sysPrompt) {
    ...
    for (const key of keys) {
        for (const model of models) {
            try {
                ...
                if (!r.ok) {
                    console.warn(`Groq ${model} ${r.status}`);
                    continue;
                }
...
async function tryGemini(messages, sysPrompt, imageData) {
    ...
    const contents = messages.map((m, idx) => {
        ...
        if (isLast && imageData) {
            ...
            if (match)
                return { role: "user", parts: [{ text: m.content }, { inlineData: { mimeType: match[1], data: match[2] } }] };
        }
...
async function tryOpenRouter(messages, sysPrompt, imageData) {
    const keys = shuffle(getEnvKeys("OPENROUTER_API_KEYS"));
    if (!keys.length)
        throw new Error("No OpenRouter keys");
    for (const key of keys) {
        try {
            const msgPayload = imageData
                ? messages.map((m, idx) => {
                    const isLast = m.role === "user" && idx === messages.length - 1;
                    if (isLast)
                        return { role: "user", content: [{ type: "text", text: m.content }, { type: "image_url", image_url: { url: imageData } }] };
            ...
            if (!r.ok) {
                console.warn(`OpenRouter ${r.status}`);
                continue;
            }
            const d = await r.json();
            const t = d.choices?.[0]?.message?.content?.trim();
...
router.post("/analyze", async (req, res) => {
    try {
        const { prompt, file, mime, userName } = req.body;
        ...
        if (isImage && fileData) {
            ...
            const extMessages = [{ role: "user", content: `${userPrompt}\n\n${fileDesc}` }];
            const providers = [
        ...
        if (!reply)
            reply = "ফাইলটি বিশ্লেষণ করতে পারলাম না। আবার চেষ্টা করো।";
        else
            reply = stripDisclaimers(reply);
        res.json({ reply });
...
router.post("/ai", async (req, res) => {
    ...
    if (!message && !imageRaw) {
        res.status(400).json({ reply: "খালি মেসেজ।" });
        return;
    }
    ...
    if (imageRaw) {
        try {
            reply = await tryGemini(msgs, SYSTEM_PROMPT, imageRaw);
        }
        catch { }
        if (!reply)
            try {
                reply = await tryOpenRouter(msgs, SYSTEM_PROMPT, imageRaw);
            }
            catch { }
    }
    else {
        try {
            reply = await tryGroq(msgs, SYSTEM_PROMPT);
        }
        catch { }
        if (!reply)
            try {
                reply = await tryGemini(msgs, SYSTEM_PROMPT);
            }
            catch { }
        if (!reply)
            try {
                reply = await tryOpenRouter(msgs, SYSTEM_PROMPT);
            }
            catch { }
    }
